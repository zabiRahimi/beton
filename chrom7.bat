...
REM Start yarn dev and redirect output to a log file
echo Starting yarn dev...
start "" cmd /c "cd C:\xampp\htdocs\beton && yarn dev > %~dp0yarn.log"

REM Wait for yarn dev to output APP_URL
echo Waiting for yarn dev to finish...
:waitYarn
findstr /m "APP_URL" %~dp0yarn.log && (
